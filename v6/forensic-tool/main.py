...
# 重点：补充 CATEGORY_KEYWORDS 的导入
from app_metadata import (
    get_app_info, get_app_icon, classify_content_enhanced,
    get_all_categories, get_category_icon, CATEGORY_KEYWORDS  # 新增这行
)
    ...
    for i in range(file_count.value):
        try:
            ...
            if content_ptr:
                ...
                category = classify_content_enhanced(content)  # 使用增强的分类函数
...
async def get_packages():
    ...
    package_info = []
    for pkg in packages:
        app_name, app_type = get_app_info(pkg)
        icon = get_app_icon(app_type)  # 根据应用类型获取图标
        package_info.append({
            "package_name": pkg,
            "app_name": app_name,
            "icon": icon
            ...
            for key in redis_cursor:
                try:
                    ...
                    if request.keyword in content:
                        ...
                        category = classify_content_enhanced(content)
                        matches.append({
                            "file_path": file_path,
                            "content": content[:500],
                            "category": category,
...
from app_metadata import CATEGORY_KEYWORDS
@app.post("/query_by_category")
async def api_query_by_category(request: QueryRequest):
    """按分类精准查询（使用分类关键词库）"""
    start_time = time.time()
    matches = []
    category = request.keyword  # 这里keyword参数传入分类名称

    # 获取该分类对应的所有关键词
    category_keywords = CATEGORY_KEYWORDS.get(category, [])
    if not category_keywords:
        return {"status": "error", "message": f"无效的分类名称：{category}", "cost_ms": 0, "count": 0, "data": []}
        ...
        try:
            ...
            for key in redis_cursor:
                try:
                    ...
                    content = value.decode(errors="ignore") if isinstance(value, bytes) else str(value)
                    # 匹配该分类下的任意关键词（忽略大小写）
                    if any(kw.lower() in content.lower() for kw in category_keywords):
                    ...
                    print(f"Redis 分类查询出错: {e}")
                    continue
        except Exception as e:
            print(f"Redis 分类查询失败: {e}")
    else:
        try:
            with db_pool.get_connection() as conn:
                db_cursor = conn.cursor()
                # 构建SQL查询条件（匹配任意关键词，忽略大小写）
                query_conditions = " OR ".join([f"LOWER(content) LIKE LOWER(?)" for _ in category_keywords])
                query_sql = f'''
                    SELECT file_path, content, package_name FROM file_data
                    WHERE {query_conditions} LIMIT 100
                '''
                # 构造参数（%关键词%）
                query_params = [f"%{kw}%" for kw in category_keywords]
                db_cursor.execute(query_sql, query_params)
                rows = db_cursor.fetchall()
                for row in rows:
                    content = row[1] if row[1] else ""
                    matches.append({
            ...
            print(f"SQLite 分类查询失败: {e}")

    # 去重（避免同一文件匹配多个关键词重复出现）
    unique_matches = []
    seen_paths = set()
    ...
    return {
        ...
        "data": unique_matches,
        "matched_keywords": category_keywords  # 返回匹配的关键词列表，方便调试
    }
